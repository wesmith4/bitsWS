...
```{r trade-balance}
showQuantityBalance <- function(country) {
  balance <- working_data %>% filter(Area == country & Item == "Potatoes") %>%
  ...
  balance %>% ggplot(mapping = aes(Year, quantity_balance)) + geom_smooth(se=FALSE) +   scale_y_continuous(label_comma())
}

showQuantityBalance("United States of America")
```
