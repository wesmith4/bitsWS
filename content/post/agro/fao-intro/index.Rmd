...
```{r usa-production-yield, layout="l-body-outset", fig.width=8}
...
crops %>% filter(Item == "Maize" & Year == 2018 & Element == "Production") %>% mutate(Rank = dense_rank(desc(value))) %>% select(Rank, Area, Item, value) %>% arrange(Rank) %>% format.data.frame(big.mark=",") %>%
  head(10) %>% kable("html", align = "c", caption = "Top 10 Producers of Corn, 2018", col.names = c("Rank", "Country", "Crop", "Production (tons)")) %>% kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
...
```{r corn-graph, fig.width=8, message=FALSE, cache=TRUE, warning=FALSE, layout="l-body-outset"}
...
crops %>% filter(Item == "Potatoes" & Year == 2018 & Element == "Production") %>% mutate(Rank = dense_rank(desc(value))) %>% select(Rank, Area, value) %>% format.data.frame(big.mark=",") %>%
  arrange(desc(value)) %>% head(10) %>% kable(align = "c", caption = "Top 10 Producers of Potatoes, 2018", col.names = c("Rank", "Country","Production (tons)")) %>% kable_styling(full_width = F, bootstrap_options = c("striped", "hover"))
```

Peru doesn't make the top-10 list, so let's see where the country stacks up in potato production.
...
Let's do another visualization of how production of potatoes in these countries has developed over time.

```{r potato-graph, message=FALSE, fig.cap="Potato Production", class.output="full-width"}
